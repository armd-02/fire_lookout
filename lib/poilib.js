						...
						default:
							...
							Conf.list.columns.act_fields.forEach(key => {
								...
								if (key.indexOf("datetime") > -1) {							// フィールド名に日時を含む場合
									data.push(basic.formatDate(new Date(line.updatetime), Conf.default.formatDate));
									...
									data.push(osmtag);
								} else if (key.indexOf(".date") > -1) {						// .dateで日付指定
									let nkey = key.substring(0,key.length - 5);					// .date抜きの元Key
									let col = line[nkey] == undefined ? "-" : line[nkey];
									if (line[nkey] !== undefined) {
										if (toString.call(new Date(col)).slice(8, -1) == "Date") {	// 日付の場合
											col = basic.formatDate(new Date(col), Conf.default.formatDate);
										}
									}
									data.push(col);
								} else {
